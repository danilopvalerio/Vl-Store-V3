  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        router.push("/dashboard"); // Ajustado para dashboard conforme nova estrutura
      }
    } catch (err) {
      ...
      const errorMessage =
        axiosError.response?.data?.message || // Tenta ler mensagem amigável
        axiosError.response?.data?.error || // Tenta ler código de erro
        "Usuário ou senha incorretos."; // Fallback
